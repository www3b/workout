<template>
  ...
  >
    <template #item="{ item }">
      ...
      >
        ...
        <template v-if="item.children && item.children.length > 0" #trailing>
          <UIcon
            :name="navigation.isItemExpanded(item.id) ? 'i-heroicons-chevron-down' : 'i-heroicons-chevron-right'"
            class="w-4 h-4 transition-transform duration-200"
            :class="{ 'rotate-90': navigation.isItemExpanded(item.id) }"
          />
        </template>
      </UButton>
    </template>
    ...
    <template #submenu="{ item, level }">
      <div
        v-if="item.children && navigation.isItemExpanded(item.id)"
